...
function downloadCsv() {
  ...
  const rows=[...data.fields,...data.engineeringSubfields,...data.computerScienceSubfields].flatMap(f=>f.metrics.map(m=>[f.id,`"${f.name}"`,f.level,`"${f.parentField||''}"`,`"${f.domain}"`,m.year,m.status,m.topPaperCount,m.forecastCount,m.top10CitedShare??'',m.cagr5??'',m.prosperityScore].join(',')));
...
export default function Home() {
  ...
  const ranking2025=useMemo(()=>[...units].sort((a,b)=>score2025(b)-score2025(a)),[units]);
  ...
  const rank=focus.metrics[25].prosperityScore===null?null:ranking2025.findIndex(f=>f.id===focus.id)+1;
  ...
  return <main>
    ...
    <section className="hero" id="top">
      <div><p className="eyebrow">GLOBAL RESEARCH PULSE · 2000—2026</p><h1>哪些科学领域，<br/><em>正在真正兴盛？</em></h1><p className="lede">从固定高影响期刊池出发，同时观察论文规模、归一化引用质量与五年增长动量。工程学与计算机科学均可下钻到具体方向。</p></div>
    ...
    <section className="data-bar"><div><p className="kicker">开放数据与复现</p><h2>每个分数，都能回到原始年度指标。</h2><p>当前版本覆盖 {data.meta.fieldCount} 个一级领域、{data.meta.engineeringSubfieldCount} 个工程子方向和 {data.meta.computerScienceSubfieldCount} 个计算机子方向。曲线默认全部展示，期刊池固定且统一采用200篇基准门槛。</p></div><div><button className="primary" onClick={downloadCsv}>下载年度 CSV</button><a href="/data/openalex.json" download>下载完整 JSON</a><button onClick={()=>setMethodOpen(true)}>查看方法说明</button></div></section>
    ...
    {methodOpen&&<div className="modal-backdrop" onMouseDown={()=>setMethodOpen(false)}><section className="method-modal" role="dialog" aria-modal="true" aria-labelledby="method-title" onMouseDown={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setMethodOpen(false)} aria-label="关闭">×</button><p className="kicker">METHODOLOGY / V1.2</p><h2 id="method-title">方法与边界</h2><h3>工程学和计算机如何拆分？</h3><p>全球视图保留完整的26个一级领域。工程学使用 Field 22 下的16个 Subfield，计算机科学使用 Field 17 下的11个 Subfield。两个下钻模式分别只在各自的同层级方向之间标准化，不与完整一级领域混排。</p><h3>顶刊如何确定？</h3><p>在每个比较单元内，从 CWTS Core 来源中筛选 2015—2019 年至少发表 200 篇论文的来源，再按高被引论文占比排序。排序使用 10% 先验、强度 100 的贝叶斯收缩。每个单元最多固定10本；符合门槛不足10本时如实减少，不降低门槛补足。</p><h3>质量如何计算？</h3><p>采用 OpenAlex 的 <code>citation_normalized_percentile</code>：按论文类型、发表年份和子领域归一化。页面展示进入前 10% 的论文比例。2024 年之后引用尚未成熟，因此标为暂定。</p><h3>2026 为什么可以展示？</h3><p>实际值截止 {data.meta.asOf}。全年预测使用该方向 2019、2022—2025 年同期发表量占全年比例的中位数进行校正；2020—2021 被排除，以减少疫情期节律异常。</p><h3>哪些结论不能直接下？</h3><p>本工具衡量的是高影响期刊论文生态，不等于全部科研活动。尤其计算机科学的重要会议论文可能被当前“期刊论文”口径低估。综合指数适合发现方向，正式判断应回看数量、质量和期刊池。</p><button className="primary full" onClick={()=>setMethodOpen(false)}>我明白了</button></section></div>}
  </main>;
}
